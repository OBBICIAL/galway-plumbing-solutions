...
export default function ImageWithFallback({
  ...
  return (
    <div className={`overflow-hidden ${fill ? 'absolute inset-0' : 'relative'} ${className || ''}`}>
      {isLoading && (
      ...
      <Image
        ...
        className={`object-cover transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'} ${fill ? '' : 'w-full h-full'}`}
        {...props}
      />
    </div>
  );
